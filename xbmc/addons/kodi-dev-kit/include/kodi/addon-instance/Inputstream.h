...
extern "C"
{
  ...
  */
  ...
  struct INPUTSTREAM_INFO
  {
    ...
    char m_name[INPUTSTREAM_MAX_STRING_NAME_SIZE]; /*!< @brief (optinal) name of the stream, \0 for default handling */
    char m_codecName[INPUTSTREAM_MAX_STRING_CODEC_SIZE]; /*!< @brief (required) name of codec according to ffmpeg */
    char m_codecInternalName
        [INPUTSTREAM_MAX_STRING_CODEC_SIZE]; /*!< @brief (optional) internal name of codec (selectionstream info) */
    enum STREAMCODEC_PROFILE m_codecProfile; /*!< @brief (optional) the profile of the codec */
    unsigned int m_pID; /*!< @brief (required) physical index */

    const uint8_t* m_ExtraData;
    unsigned int m_ExtraSize;

    char m_language[INPUTSTREAM_MAX_STRING_LANGUAGE_SIZE]; /*!< @brief RFC 5646 language code (empty string if undefined) */
    ...
    enum INPUTSTREAM_COLORSPACE m_colorSpace; /*!< @brief definition of colorspace */
    enum INPUTSTREAM_COLORRANGE m_colorRange; /*!< @brief color range if available */
    ...
    struct INPUTSTREAM_MASTERING_METADATA* m_masteringMetadata; /*!< @brief mastering static Metadata */
    struct INPUTSTREAM_CONTENTLIGHT_METADATA*
  ...
  typedef struct KodiToAddonFuncTable_InputStream /* internal */
  {
    ...
    bool(__cdecl* open)(const struct AddonInstance_InputStream* instance, struct INPUTSTREAM_PROPERTY* props);
    ...
    // ITime
    bool(__cdecl* get_times)(const struct AddonInstance_InputStream* instance, struct INPUTSTREAM_TIMES* times);
...
private:
  ...
  inline static bool ADDON_Open(const AddonInstance_InputStream* instance, INPUTSTREAM_PROPERTY* props)
  {
